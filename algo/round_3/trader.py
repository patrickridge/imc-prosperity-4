...
# --- VEV options: passive bid MM ---
# Keep the style passive, but anchor bids to the underlying and give ourselves
# an aggressive exit when the option is clearly rich to model.
_VEV_BID_EDGES: Dict[str, float] = {
    "VEV_4000": 8.0,   # spread=21, post inside
    "VEV_5200": 0.05,  # spread=3, mid=96.5 → posts at 96 (inside spread)
    "VEV_5300": 0.05,  # spread=2, mid=48 → posts at 47 (at existing bid)
    "VEV_5400": 0.05,  # spread=1, cheapest IV in near-ATM cluster
}
...
_VEV_LOTS: Dict[str, int] = {
    ...
    "VEV_5200": 20,
    "VEV_5300": 20,
    # Lowest-delta active option; we can carry slightly more size here
    # without materially increasing portfolio delta.
    "VEV_5400": 30,
}
_VEV_GROSS_CAP = 800
...
# --- Hydrogel (401011): anchored MM, symmetric take edge ---
...
# --- Velvetfruit (399751): simple delta mean-reversion from 420919 ---
...
HYDROGEL_CONFIG = ProductConfig(
    ...
    max_skew=6,  # was 3; larger skew fights extreme inventory buildup
    quote_size=40,
)
...
# ── Hydrogel: anchored MM from 401011 ──
...
# ── Velvetfruit: simple delta mean-reversion from 420919 ──
...
def _trade_vev(
    ...
    # If we're already long and the bid is clearly above model, hit it and
    # shrink risk instead of waiting for passive asks that rarely fill.
...
# ── Hydrogel execution infrastructure (401011) with corrected live limit compliance ──
...
def _trade_dynamic_product(
    ...
    buy_sent = 0
    sell_sent = 0
    # Live validates worst-case exposure against current position, not projected
    ...
    for ask_price in sorted(od.sell_orders):
        ...
        qty = min(ask_qty, buy_cap - buy_sent, target_cap - projected)
        if qty > 0:
    ...
    for bid_price in sorted(od.buy_orders, reverse=True):
        ...
        qty = min(bid_qty, sell_cap - sell_sent, projected - target_floor)
        if qty > 0:
            orders.append(Order(config.symbol, bid_price, -qty))
            projected -= qty
            sell_sent += qty

    projected, buy_sent, sell_sent = _clear_inventory(
        config,
        od,
        orders,
        projected,
...
def _clear_inventory(
    ...
    if projected > 0 and fair_ask in od.buy_orders:
        qty = min(projected, od.buy_orders[fair_ask], sell_cap - sell_sent)
        if qty > 0:
    ...
    if projected < 0 and fair_bid in od.sell_orders:
        qty = min(-projected, -od.sell_orders[fair_bid], buy_cap - buy_sent)
        if qty > 0:
            orders.append(Order(config.symbol, fair_bid, qty))
            projected += qty
            buy_sent += qty

    return projected, buy_sent, sell_sent
...
def _post_quotes(
    ...
    buy_qty = min(config.quote_size, buy_cap - buy_sent)
    if buy_qty > 0:
        orders.append(Order(config.symbol, bid_price, buy_qty))

    sell_qty = min(config.quote_size, sell_cap - sell_sent)
    if sell_qty > 0:
