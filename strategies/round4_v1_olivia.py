...
_VEV_ENTRY_EDGE: Dict[str, float] = {
    "VEV_4000": 3.0,
    "VEV_4500": 5.0,   # was 3.0 — option C: tighter (=larger) edge filters marginal trades
    "VEV_5000": 4.0,   # was 2.0
    "VEV_5100": 3.0,   # was 1.5
    "VEV_5200": 2.0,   # was 1.0
    "VEV_5300": 1.5,   # was 0.7
    "VEV_5400": 1.0,   # was 0.5
    "VEV_5500": 0.4,
}
...
_VEV_MM_BID_STRIKES = ("VEV_4000", "VEV_4500")  # Idea 2: add 4500 as second bid wing
_VEV_MM_ASK_STRIKES = ("VEV_5500",)
_VEV_MM_BID_OFFSET: Dict[str, float] = {"VEV_4000": 8.0, "VEV_4500": 6.0}
...
# Option B: exit-side quotes for wings — turns one-sided MM into two-sided MM.
# Wide offset so we only get filled on real reversals (recycle inventory, not bleed).
...
# Option B: inner strikes proven to be ANTI-alpha on the buy side (every losing
# strike on IMC 480742 was pinned at +pos_cap = bought into a falling market).
# These strikes are SELL-ONLY for mean-rev. The sell leg is unobserved (untested),
# but worst case it just doesn't fire and we stay flat.
...
HYDROGEL_TARGET_SCALE = 25  # was 40 — Idea 1: more aggressive sizing on signal
HYDROGEL_MAX_TAKE_SIZE = 20  # was 10 — Idea 1: scale up proven IMC alpha

VELVETFRUIT_ANCHOR_SEED = 5_255.0
VELVETFRUIT_ANCHOR_ALPHA = 0.002
VELVETFRUIT_LIMIT = 200
VELVETFRUIT_ENTRY_EDGE = 15.0  # was 10.0 — Idea 3: filter marginal trades that bleed on IMC
VELVETFRUIT_CLIP = 20

# ---------- ROUND 4 informed-trader overlay -------------------------------
# Discovered from R4 trade data (research/find_olivia.py):
#   Mark 14 = HYDROGEL informed  -> after buy: mid +1.06/+0.95 across days 1-2
#                                    after sell: mid -1.56/-0.47/-1.32 across days 1-3
#   Mark 67 = VELVET informed (LONG-ONLY) -> 165 buys, 0 sells across 3 days
#                                            after buy: mid +2.95/+1.62/+1.01
# When signal active, aggressively flip our position toward the limit.
HYDROGEL_INFORMED_TRADER = "Mark 14"
VELVET_INFORMED_TRADER = "Mark 67"
INFORMED_SIGNAL_WINDOW = 10_000   # timestamp ticks signal stays "live" after a trade
INFORMED_TARGET_FRACTION = 1.0    # full ±limit on signal — proven best in sweep
INFORMED_TAKE_DEPTH = 5            # walk this many ask/bid levels when copy-trading
...
def _update_informed_signals(data: dict, state: TradingState) -> None:
    """Stamp last buy/sell timestamps for the two informed traders we track."""
    hydro_trades = state.market_trades.get(HYDROGEL_PACK, [])
    velvet_trades = state.market_trades.get(VELVETFRUIT_EXTRACT, [])
    for tr in hydro_trades:
        if tr.buyer == HYDROGEL_INFORMED_TRADER:
            data["hydrogel_informed_buy_ts"] = tr.timestamp
        if tr.seller == HYDROGEL_INFORMED_TRADER:
            data["hydrogel_informed_sell_ts"] = tr.timestamp
    for tr in velvet_trades:
        if tr.buyer == VELVET_INFORMED_TRADER:
            data["velvet_informed_buy_ts"] = tr.timestamp


def _informed_direction(data: dict, key_prefix: str, now: int) -> int:
    """Return +1 (long bias), -1 (short bias) or 0 (neutral)."""
    last_buy = data.get(f"{key_prefix}_buy_ts")
    last_sell = data.get(f"{key_prefix}_sell_ts")
    buy_live = last_buy is not None and now - last_buy <= INFORMED_SIGNAL_WINDOW
    sell_live = last_sell is not None and now - last_sell <= INFORMED_SIGNAL_WINDOW
    if buy_live and sell_live:
        return 1 if last_buy >= last_sell else -1
    if buy_live:
        return 1
    if sell_live:
        return -1
...
def _walk_take_buy(symbol: str, od: OrderDepth, qty_left: int, levels: int) -> List[Order]:
    """Sweep levels-deep ask side, taking up to qty_left contracts total."""
    out: List[Order] = []
    if qty_left <= 0:
        return out
    for ask_price in sorted(od.sell_orders)[:levels]:
        avail = -od.sell_orders[ask_price]
        if avail <= 0:
            continue
        take = min(avail, qty_left)
        out.append(Order(symbol, ask_price, take))
        qty_left -= take
        if qty_left <= 0:
            break
    return out
...
def _walk_take_sell(symbol: str, od: OrderDepth, qty_left: int, levels: int) -> List[Order]:
    out: List[Order] = []
    if qty_left <= 0:
        return out
    for bid_price in sorted(od.buy_orders, reverse=True)[:levels]:
        avail = od.buy_orders[bid_price]
        if avail <= 0:
            continue
        take = min(avail, qty_left)
        out.append(Order(symbol, bid_price, -take))
        qty_left -= take
        if qty_left <= 0:
            break
    return out
...
def _trade_hydrogel_with_signal(
    od: OrderDepth, position: int, fair_value: float, signal: int
) -> List[Order]:
    if signal == 0:
        return _trade_hydrogel(od, position, fair_value)
    # Bias the target position toward the limit in the signal direction.
    # Reuse the proven take/post pipeline so we don't pay through the spread.
    target_position = int(signal * HYDROGEL_LIMIT * INFORMED_TARGET_FRACTION)
...
def _trade_velvetfruit_with_signal(
    od: OrderDepth, position: int, anchor: float, signal: int
) -> List[Order]:
    """Run baseline mean-rev. When Mark 67 just bought, also lift the ask if
    it's at-or-below anchor (free or near-free to enter). Don't pay premium.
    """
    base_orders = _trade_velvetfruit(od, position, anchor)
    if signal != 1:
        return base_orders
    bid = _best_bid(od)
    ask = _best_ask(od)
    if bid is None or ask is None:
        return base_orders
    # Only act if the ask is at or below anchor — cheap-or-flat entries only.
    if ask > anchor:
        return base_orders
    # Skip if baseline already issued a buy at this price.
    if any(o.price == ask and o.quantity > 0 for o in base_orders):
        return base_orders
    headroom = VELVETFRUIT_LIMIT - position
    if headroom <= 0:
        return base_orders
    qty = min(headroom, abs(od.sell_orders[ask]), VELVETFRUIT_CLIP)
    if qty > 0:
        base_orders.append(Order(VELVETFRUIT_EXTRACT, ask, qty))
    return base_orders
...
class Trader:
    def run(
        ...
        _update_informed_signals(data, state)
        hydrogel_signal = _informed_direction(
            data, "hydrogel_informed", state.timestamp
        )
        velvet_signal = _informed_direction(
            data, "velvet_informed", state.timestamp
        )
        ...
        if "HYDROGEL" in _ENABLED_PRODUCTS and HYDROGEL_PACK in state.order_depths:
            result[HYDROGEL_PACK] = _trade_hydrogel_with_signal(
        ...
        if "VELVETFRUIT" in _ENABLED_PRODUCTS and VELVETFRUIT_EXTRACT in state.order_depths:
            # VELVET overlay disabled: net-negative across days 1+3. Baseline only.
        ...
        # Pairwise no-arb sweep — runs LAST (matches friend's order). Mops up
        # leftover capacity after per-strike strategies have placed their bets,
        # respecting their orders via _arb_capacity().
...
def _vev_passive_bid(sym: str, od: OrderDepth, position: int) -> List[Order]:
    ...
    # Exit side: only post when we've accumulated enough inventory to recycle.
...
def _vev_passive_ask(sym: str, od: OrderDepth, position: int) -> List[Order]:
    ...
    # Exit side: only post when we've accumulated enough short inventory to recycle.
    if position <= -_VEV_MM_EXIT_TRIGGER[sym]:
...
def _vev_mean_revert(
    ...
    # Option B: inner strikes are sell-only — buy leg is anti-alpha on IMC.
