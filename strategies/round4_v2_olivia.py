...
# Round 4 informed-trader overlay (HYDROGEL only).
# Mark 14 buys -> mid drifts +1 in next ~10k ticks (proven on R4 days 1-3).
# Mark 14 sells -> mid drifts -1.5. We bias HYDROGEL target to ±LIMIT.
HYDROGEL_INFORMED_TRADER = "Mark 14"
INFORMED_SIGNAL_WINDOW = 10_000
INFORMED_TARGET_FRACTION = 1.0
...
def _trade_hydrogel(
    od: OrderDepth, position: int, signal: int = 0,
) -> List[Order]:
    if signal != 0:
        target_position = int(
            signal * HYDROGEL_CONFIG.limit * INFORMED_TARGET_FRACTION
        )
    else:
        target_position = _hydrogel_baseline_target(od)
...
def _hydrogel_baseline_target(od: OrderDepth) -> int:
