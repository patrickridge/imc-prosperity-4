...
class Trader:
    ...
    SOLAR_FLAMES_WINDOW = 1500
    SOLAR_FLAMES_ENTRY_DROP = -400.0
    ...
    DARK_MATTER_WINDOW = 1500
    DARK_MATTER_ENTRY_DROP = -200.0
    # v9: also allow delayed long carry after 5000 ticks.
    # This keeps the original fast dip-buy, but stops missing the later positive carry regime.
    ENABLE_DARK_MATTER_DELAYED_LONG = True
    DARK_MATTER_DELAY_WARMUP = 5000
    ...
    PLANETARY_RINGS_WINDOW = 3000
    PLANETARY_RINGS_ENTRY_DROP = -150.0
    PLANETARY_RINGS_TARGET = -10

    ENABLE_SOLAR_WINDS_BREAKDOWN_SHORT = True
    SOLAR_WINDS_WINDOW = 5000
    SOLAR_WINDS_ENTRY_DROP = -25.0
    ...
    ENABLE_MORNING_LATE_LONG = True
    # v9: adaptive late dip-buy instead of pure delay.
    # Go long after 8000 ticks if the 8000-tick slope is still below +25.
    MORNING_WINDOW = 8000
    MORNING_ENTRY_SLOPE = 25.0
    MORNING_TARGET = 10
    ...
    def compute_targets(self, data: Dict[str, Any]) -> Dict[str, int]:
        ...
        if self.ENABLE_DARK_MATTER_DIP_BUY:
            dark_target = self.slope_threshold_target(
                data,
                self.DARK_MATTER,
                self.DARK_MATTER_WINDOW,
                self.DARK_MATTER_ENTRY_DROP,
                self.DARK_MATTER_TARGET,
                direction="below",
            )
            if dark_target == 0 and self.ENABLE_DARK_MATTER_DELAYED_LONG:
                dark_target = self.warmup_target(
                    data,
                    self.DARK_MATTER,
                    self.DARK_MATTER_DELAY_WARMUP,
                    self.DARK_MATTER_TARGET,
                )
            targets[self.DARK_MATTER] = dark_target
        ...
        if self.ENABLE_MORNING_LATE_LONG:
            targets[self.MORNING] = self.slope_threshold_target(
                data,
                self.MORNING,
                self.MORNING_WINDOW,
                self.MORNING_ENTRY_SLOPE,
                self.MORNING_TARGET,
                direction="below",
            )
